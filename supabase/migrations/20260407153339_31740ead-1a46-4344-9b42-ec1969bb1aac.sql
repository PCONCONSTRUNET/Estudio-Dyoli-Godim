...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Handle status changes: cancel/no-show removes blocks
    ...
    IF NEW.status IN ('pendente', 'confirmado', 'concluido') AND OLD.status IN ('cancelado', 'falta') THEN
      ...
      END LOOP;
    END IF;
    -- Handle duration changes: recalculate blocked slots
    IF NEW.duracao_minutos IS DISTINCT FROM OLD.duracao_minutos AND NEW.status IN ('pendente', 'confirmado', 'concluido') THEN
      DELETE FROM horarios_bloqueados WHERE motivo = 'agendamento:' || ref_id;
      FOR i IN 0..(slot_count - 1) LOOP
        slot_hour := start_hour + ((start_minute + (i * 30)) / 60);
        slot_minute := (start_minute + (i * 30)) % 60;
