-- Function to manage blocked slots when agendamento changes
...
BEGIN
  ...
  slot_count := CEIL(ref_duracao::NUMERIC / 60);

  -- On INSERT: block slots if status is active
  IF TG_OP = 'INSERT' THEN
    IF NEW.status IN ('pendente', 'confirmado', 'concluido') THEN
      FOR i IN 0..(slot_count - 1) LOOP
        slot_hour := start_hour + ((start_minute + (i * 60)) / 60);
        slot_minute := (start_minute + (i * 60)) % 60;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    END IF;
    IF NEW.status IN ('pendente', 'confirmado', 'concluido') AND OLD.status IN ('cancelado', 'falta') THEN
      FOR i IN 0..(slot_count - 1) LOOP
        slot_hour := start_hour + ((start_minute + (i * 60)) / 60);
        slot_minute := (start_minute + (i * 60)) % 60;
        slot_time := LPAD(slot_hour::TEXT, 2, '0') || ':' || LPAD(slot_minute::TEXT, 2, '0');
...
DROP TRIGGER IF EXISTS trg_manage_blocked_slots ON agendamentos;
CREATE TRIGGER trg_manage_blocked_slots
